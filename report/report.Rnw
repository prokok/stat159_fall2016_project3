...
variable_info <- data.frame(colnames(predictors[,4:(length(predictors)-2)]), c("Black Student Body Percentage", "Hispanic Student Body Percentage", "5-year Overall Completition Rate", "Net Price (Public and Private Schools Combined)", "Admission Rate", "4-Year Retention Rate", "Median Post-Graduate Debt", "Percentage of Student Body on Pell Grants", "Median Post-Graduate Earnings", "Total Graduation Rate"))
...
For both minoritiy groups we looked at, Ridge regression far outperformed OLS, so we used our ridge model throughout. 
